...
def _copilot_runtime_api_mode(
    ...
    # Only honor persisted api_mode when no explicit target model overrides
    # the config default — a stale api_mode from the previous model must not
    # leak onto a mid-session/one-shot model switch (same bug class as
    # opencode-zen/go re-derivation below).
    if not target_model:
        configured_mode = _parse_api_mode(model_cfg.get("api_mode"))
